...
fn addresses(is_liquid: bool) {
    ...
    // generate some addresses and receive funds on them
    // so the address index is increased
    let mut last_txid = "".to_string();
    for i in 1..12 {
        let ap = loop {
            // address index is reset if not used,
            // loop until the tx is received and it's updated
            let ap = test_session.get_receive_address(0);
            if ap.pointer == i {
                break ap;
            }
        };
        last_txid = test_session.node_sendtoaddress(&ap.address, 1000, None);
    }

    test_session.wait_tx(vec![0], &last_txid, Some(1000), Some(TransactionType::Incoming));
