...
namespace sdk {
    ...
    void ga_session::encache_new_scriptpubkeys(uint32_t subaccount)
    {
        ...
        auto details = nlohmann::json({ { "subaccount", subaccount }, { "last_pointer", nullptr } });
        {
            locker_t locker(m_mutex);
        ...
        do {
            ...
            if (details["last_pointer"].is_null()) {
    ...
    nlohmann::json ga_session::get_previous_addresses(const nlohmann::json& details)
    {
        ...
        if (details.contains("last_pointer") && !details["last_pointer"].is_null()) {
            last_pointer = details["last_pointer"];
        }
        if (!details["last_pointer"].is_null() && last_pointer < 2) {
            // Prevent a server call if the user iterates until empty results
            return { { "subaccount", subaccount }, { "list", nlohmann::json::array() }, { "last_pointer", nullptr } };
        }
        ...
        if (seen_pointer < 2) {
            return nlohmann::json{ { "subaccount", subaccount }, { "last_pointer", nullptr }, { "list", addresses } };
        }
